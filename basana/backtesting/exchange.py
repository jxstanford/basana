...
def assert_has_value(balance_updates: Dict[str, Decimal], symbol: str, sign: Decimal):
    ...
    assert (
        bt_helpers.get_sign(value) == sign
    ), f"{symbol} sign is wrong. It should be {sign}"
...
class Exchange:
    ...
    :param fee_strategy: The fee strategy to use.
    ...
    async def create_limit_order(
        ...
        return await self.create_order(
            requests.LimitOrder(operation, pair, amount, limit_price)
        )
    ...
    async def create_stop_order(
        ...
        * A sell stop order is entered at a stop price below the current market price. Investors generally use a sell
        ...
        """
        return await self.create_order(
            requests.StopOrder(operation, pair, amount, stop_price)
        )
    ...
    async def create_stop_limit_order(
        ...
        """
        return await self.create_order(
            requests.StopLimitOrder(operation, pair, amount, stop_price, limit_price)
        )
    ...
    async def cancel_order(self, order_id: str) -> CanceledOrder:
        ...
        if not order.is_open:
            raise Error(
                "Order {} is in {} state and can't be canceled".format(
    ...
    def _round_balance_updates(
        self, pair: Pair, balance_updates: Dict[str, Decimal]
    ) -> Dict[str, Decimal]:
        ...
        if base_amount:
            base_amount = core_helpers.truncate_decimal(
                base_amount, pair_info.base_precision
            )
        ...
        if quote_amount:
            quote_amount = core_helpers.round_decimal(
                quote_amount, pair_info.quote_precision
            )
    ...
    def _round_fees(self, pair: Pair, fees: Dict[str, Decimal]) -> Dict[str, Decimal]:
        ...
        for symbol in ret.keys():
            ...
            if amount and (precision := precisions.get(symbol)) is not None:
                amount = core_helpers.round_decimal(
                    amount, precision, rounding=decimal.ROUND_UP
                )
                ret[symbol] = amount
    ...
    def _process_orders(self, bar_event: bar.BarEvent):
        if (
            liquidity_strategy := self._liquidity_strategies.get(bar_event.bar.pair)
        ) is None:
            liquidity_strategy = self._liquidity_strategy_factory()
        liquidity_strategy.on_bar(bar_event.bar)
        for order in filter(
            lambda o: o.pair == bar_event.bar.pair, self._orders.get_open_orders()
        ):
    ...
    async def _on_bar_event(self, event: event.Event):
        assert isinstance(
            event, bar.BarEvent
        ), f"{event} is not an instance of bar.BarEvent"
    ...
    def _check_available_balance(self, required_balance: Dict[str, Decimal]):
        for symbol, required in required_balance.items():
            ...
            if available_balance < required:
                raise errors.Error(
                    "Not enough {} available. {} are required and {} are available".format(
...
class FuturesExchange(Exchange):
    ...
    def _get_contract_info(self, contract: Contract) -> PairInfo:
        ...
        if ret is None:
            ret = self._default_contract_info
        return ret
    ...
    async def create_market_order(
        ...
        assert isinstance(contract, Contract)
        return await self.create_order(
            requests.MarketFuturesOrder(operation, contract, quantity)
        )
    ...
    async def create_limit_order(
        ...
        assert isinstance(contract, Contract)
        return await self.create_order(
            requests.LimitFuturesOrder(operation, contract, amount, limit_price)
        )
