...
class ExchangeOrder(metaclass=abc.ABCMeta):
    ...
    def validate(self, pair_info: PairInfo):
        ...
        if self.amount != helpers.truncate_decimal(
...
class MarketOrder(ExchangeOrder):
    ...
    def create_order(self, id: str) -> orders.Order:
        return orders.MarketOrder(
            id, self.operation, self.pair, self.amount, orders.OrderState.OPEN
        )
...
class LimitOrder(ExchangeOrder):
    ...
    def validate(self, pair_info: PairInfo):
        ...
        if self.limit_price != helpers.truncate_decimal(
            self.limit_price, pair_info.quote_precision
        ):
            raise errors.Error(
                "{} exceeds maximum precision of {} decimal digits".format(
                    self.limit_price, pair_info.quote_precision
...
class StopOrder(ExchangeOrder):
    ...
    def validate(self, pair_info: PairInfo):
        ...
        if self.stop_price != helpers.truncate_decimal(
            self.stop_price, pair_info.quote_precision
        ):
            raise errors.Error(
                "{} exceeds maximum precision of {} decimal digits".format(
                    self.stop_price, pair_info.quote_precision
    ...
    def validate(self, pair_info: PairInfo):
        ...
        for value in [self.stop_price, self.limit_price]:
            if value != helpers.truncate_decimal(value, pair_info.quote_precision):
                raise errors.Error(
                    "{} exceeds maximum precision of {} decimal digits".format(
                        value, pair_info.quote_precision
                    )
                )
...
class FuturesExchangeOrder(ExchangeOrder):
    def __init__(
        self, operation: OrderOperation, contract: Contract, quantity: Decimal
    ):
...
class MarketFuturesOrder(FuturesExchangeOrder):
    """Futures exchange market order request.
...
class LimitFuturesOrder(FuturesExchangeOrder):
    """Futures exchange Limit order request.
...
class StopFuturesOrder(FuturesExchangeOrder):
    """Futures exchange Stop order request.
