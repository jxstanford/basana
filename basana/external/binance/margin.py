...
class Account(metaclass=abc.ABCMeta):
    ...
    async def create_order(
        self, order_request: margin_requests.ExchangeOrder
    ) -> CreatedOrder:
    ...
    async def get_order_info(
        ...
        if include_trades:
            trades = [
                Trade(trade)
                for trade in await self.client.get_trades(
    ...
    async def get_open_orders(self, pair: Optional[Pair] = None) -> List[OpenOrder]:
        ...
        return [
            ...
            for open_order in await self.client.get_open_orders(order_book_symbol)
        ]
