...
class Account:
    ...
    async def get_balances(self) -> Dict[str, Balance]:
        ...
        return {
            balance["asset"].upper(): Balance(balance)
            for balance in account_info["balances"]
        }

    async def create_order(
        self, order_request: spot_requests.ExchangeOrder
    ) -> CreatedOrder:
    ...
    async def get_order_info(
        ...
        if include_trades:
            trades = [
                Trade(trade)
                for trade in await self._cli.get_trades(
                    order_book_symbol, order_id=order_info["orderId"]
                )
            ]
    ...
    async def get_open_orders(self, pair: Optional[Pair] = None) -> List[OpenOrder]:
        ...
        return [
            ...
            for open_order in await self._cli.get_open_orders(order_book_symbol)
        ]
