...
def test_account_balances(backtesting_dispatcher):
    async def impl():
        e = exchange.Exchange(
            ...
            {"usd": Decimal("1000"), "ars": Decimal("-2000.05"), "eth": Decimal("0")},
        )
...
def test_create_get_and_cancel_order(backtesting_dispatcher):
    async def impl():
        ...
        order_request = requests.MarketOrder(
            exchange.OrderOperation.BUY, Pair("BTC", "USD"), Decimal("1")
        )
...
def test_bar_events_from_csv_and_backtesting_log_mode(backtesting_dispatcher, caplog):
    ...
    async def impl():
        ...
        p = Pair("IBM", "USD")
        e.add_bar_source(
            bars.CSVBarSource(p, abs_data_path("orcl-2001-yahoo.csv"), sort=True)
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        requests.MarketOrder(
            exchange.OrderOperation.BUY, Pair("ORCL", "USD"), Decimal(0)
        ),
        requests.MarketOrder(
            exchange.OrderOperation.BUY, Pair("ORCL", "USD"), Decimal(-1)
        ),
        requests.MarketOrder(
            exchange.OrderOperation.BUY, Pair("ORCL", "USD"), Decimal("0.1")
        ),
        requests.MarketOrder(
            exchange.OrderOperation.BUY, Pair("ORCL", "USD"), Decimal("1.1")
        ),
        requests.MarketOrder(
            exchange.OrderOperation.BUY, Pair("BTC", "USD"), Decimal("1.000000001")
        ),
        # Limit order: Invalid amount/price.
        requests.LimitOrder(
            exchange.OrderOperation.BUY, Pair("ORCL", "USD"), Decimal(0), Decimal("1")
        ),
        requests.LimitOrder(
            exchange.OrderOperation.BUY, Pair("ORCL", "USD"), Decimal(1), Decimal("0")
        ),
        requests.LimitOrder(
            exchange.OrderOperation.BUY,
            Pair("ORCL", "USD"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        requests.LimitOrder(
            exchange.OrderOperation.SELL, Pair("ORCL", "USD"), Decimal(1), Decimal("1")
        ),
        requests.StopOrder(
            exchange.OrderOperation.BUY,
            Pair("ORCL", "USD"),
...
def test_small_fill_is_ignored_after_rounding(backtesting_dispatcher):
    ...
    async def impl():
        created_order = await e.create_order(
            requests.LimitOrder(
                exchange.OrderOperation.BUY, p, Decimal("0.1"), Decimal("2")
            )
        )
        await backtesting_dispatcher.run()
...
def test_liquidity_is_exhausted_and_order_is_canceled(backtesting_dispatcher):
    ...
    async def impl():
        ...
        created_order_1 = await e.create_order(
            requests.MarketOrder(
                exchange.OrderOperation.BUY, Pair("ORCL", "USD"), amount_1
            )
        )
        ...
        created_order_2 = await e.create_order(
            requests.MarketOrder(
                exchange.OrderOperation.BUY, Pair("ORCL", "USD"), amount_2
            )
        )
        await backtesting_dispatcher.run()
...
def test_balance_is_on_hold_while_order_is_open(backtesting_dispatcher):
    async def impl():
        ...
        created_order_1 = await e.create_order(
            requests.LimitOrder(
                exchange.OrderOperation.BUY, p, Decimal(1), Decimal(750)
            )
        )
