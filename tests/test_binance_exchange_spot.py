    ...
    "orders": [
        {
            "symbol": "BTCUSDT",
            "orderId": 15558250268,
            "clientOrderId": "ZDlLvguLRpgGRfcwLLeATp",
        },
        {
            "symbol": "BTCUSDT",
            "orderId": 15558250269,
            "clientOrderId": "AdLeXmt1ChmIloCJJKC0xu",
        },
    ],
    "orderReports": [
        {
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {
                "id": "15374780716",
                "datetime": datetime.datetime(2022, 11, 11, 1, 11, 10, 269000).replace(
        ...
        (
            ...
            {
                "id": "15455625561",
                "datetime": datetime.datetime(2022, 11, 13, 2, 34, 35, 519000).replace(
                    tzinfo=datetime.timezone.utc
                ),
        ...
        (
            ...
            {
                "id": "15456494165",
                "datetime": datetime.datetime(2022, 11, 13, 3, 20, 13, 411000).replace(
                    tzinfo=datetime.timezone.utc
                ),
...
def test_create_order(
    ...
    binance_http_api_mock.post(
        re.compile(r"http://binance.mock/api/v3/order\\?.*"),
        status=200,
...
def test_order_info(
    ...
    binance_http_api_mock.get(
        re.compile(r"http://binance.mock/api/v3/myTrades\\?.*"),
        status=200,
...
def test_error_retrieving_order_info(binance_http_api_mock, binance_exchange):
    binance_http_api_mock.get(
        re.compile(r"http://binance.mock/api/v3/order\\?.*"), status=500, payload={}
    )

    async def test_main():
        with pytest.raises(exchange.Error) as excinfo:
            await binance_exchange.spot_account.get_order_info(
                pair.Pair("BTC", "USDT"), order_id="1234"
            )
    ...
    [
        (
            ...
            ],
            {
                ...
                "status": "NEW",
                "time_in_force": "GTC",
                "datetime": datetime.datetime(2023, 2, 15, 17, 34, 15, 273000).replace(
                    tzinfo=datetime.timezone.utc
                ),
                "type": "LIMIT",
            },
...
def test_get_open_orders(
    pair,
    open_orders_payload,
    expected_first_open_order,
    binance_http_api_mock,
    binance_exchange,
):
    binance_http_api_mock.get(
        re.compile(r"http://binance.mock/api/v3/openOrders\\?.*"),
        status=200,
    ...
    [
        (
            ...
            },
            {
                ...
                "datetime": datetime.datetime(2022, 11, 15, 2, 21, 7, 902000).replace(
                    tzinfo=datetime.timezone.utc
                ),
                "is_open": True,
...
def test_create_oco_order(
    ...
    binance_http_api_mock.post(
        re.compile(r"http://binance.mock/api/v3/order/oco\\?.*"),
        status=200,
        payload=response_payload,
    )
    ...
    [
        (
            ...
            CANCELED_OCO_ORDER_RESPONSE,
            {
                ...
                "datetime": datetime.datetime(2022, 11, 15, 15, 53, 3, 935000).replace(
                    tzinfo=datetime.timezone.utc
                ),
                "is_open": False,
            },
...
def test_cancel_oco_order(
    ...
    binance_http_api_mock.delete(
        re.compile(r"http://binance.mock/api/v3/orderList\\?.*"),
        status=200,
        payload=response_payload,
    )
