...
def test_daily_bars_from_csv(backtesting_dispatcher):
    ...
    async def impl():
        ...
        src = csv_bars.BarSource(
            pair, abs_data_path("bitstamp_btcusd_day_2015.csv"), "1d"
        )
        ...
        assert bars[-1].datetime == datetime.datetime(
            2015, 12, 31, tzinfo=datetime.timezone.utc
        )
        assert bars[-1].open == Decimal("426.09")
        assert events[-1].when == datetime.datetime(
            2015, 12, 31, 23, 59, 59, 999999, tzinfo=datetime.timezone.utc
        )
...
def test_minute_bars_from_csv_using_deprecated_period_format(backtesting_dispatcher):
    ...
    async def impl():
        ...
        src = csv_bars.BarSource(
            pair,
            abs_data_path("bitstamp_btcusd_min_2020_01_01.csv"),
            csv_bars.BarPeriod.MINUTE,
        )
        backtesting_dispatcher.subscribe(src, on_bar)
        await backtesting_dispatcher.run()
        ...
        assert (
            len(bars) == 1440 - 45
        )  # There are 45 bars with no volume that are skipped.
        ...
        assert bars[-1].datetime == datetime.datetime(
            2020, 1, 1, 23, 59, tzinfo=datetime.timezone.utc
        )
