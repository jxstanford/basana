...
def test_open_orders(bitstamp_http_api_mock, bitstamp_exchange):
    async def test_main():
        for p in [None, pair.Pair("ETH", "USD")]:
            ...
            assert open_order.datetime == datetime.datetime(
                2022, 9, 20, 16, 11, 6
...
def test_order_info(
    order_id, expected_attrs, bitstamp_http_api_mock, bitstamp_exchange
):
    async def test_main():
        order_info = await bitstamp_exchange.get_order_info(
            pair.Pair("USDT", "USD"), order_id=order_id
        )
        assert order_info is not None

        if "fill_price" in expected_attrs:
            assert (
                helpers.truncate_decimal(order_info.fill_price, 5)
                == expected_attrs["fill_price"]
            )
            del expected_attrs[
                "fill_price"
            ]  # So its not checked inside assert_expected_attrs
...
def test_order_status(bitstamp_http_api_mock, bitstamp_exchange):
    async def test_main():
        order_status = await bitstamp_exchange.get_order_status(
            order_id="1536137941123072"
        )
        ...
        assert (
            order_status.transactions[0].type == exchange.TransactionType.MARKET_TRADE
        )
...
def test_order_status_without_transactions(bitstamp_http_api_mock, bitstamp_exchange):
    async def test_main():
        order_status = await bitstamp_exchange.get_order_status(
            order_id="1536137941123072", omit_transactions=True
        )
        assert order_status is not None
        assert order_status.id == "1536137941123072"
        assert order_status.status == "Canceled"
...
def test_cancel_order(bitstamp_http_api_mock, bitstamp_exchange):
    async def test_main():
        ...
        assert helpers.truncate_decimal(canceled_order.amount, 8) == Decimal(
            "0.00319028"
        )
...
def test_order_requests(
    ...
    async def test_main():
        ...
        assert order_created.datetime == datetime.datetime(
            2022, 9, 30, 16, 47, 12, 583000
        ).replace(tzinfo=datetime.timezone.utc)
...
def test_get_pair_info(bitstamp_http_api_mock, bitstamp_exchange):
    async def impl():
        assert await bitstamp_exchange.get_pair_info(
            pair.Pair("BTC", "USD")
        ) == pair.PairInfo(8, 0)
